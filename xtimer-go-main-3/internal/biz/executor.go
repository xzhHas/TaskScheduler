...
func (w *ExecutorUseCase) execute(ctx context.Context, timer *Timer) (map[string]interface{}, error) {
	...
	// 只允许 GET 和 POST 请求
	switch notifyHTTPParam.Method {
	case "GET":
		// 直接打开浏览器访问 URL
		err = utils.OpenURL(notifyHTTPParam.Url)
	case "POST", "": // 默认为 POST
		err = w.httpClient.Post(ctx, notifyHTTPParam.Url, notifyHTTPParam.Header, notifyHTTPParam.Body, &resp)
	default:
		return nil, fmt.Errorf("unsupported HTTP method: %s", notifyHTTPParam.Method)
	}
...
func (w *ExecutorUseCase) postProcess(ctx context.Context, resp map[string]interface{}, execErr error, app string, timerID uint, unix int64, execTime time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("get task failed, timerID: %d, runTimer: %d, err: %w\n", timerID, time.UnixMilli(unix), err)
	}
	...
	if err != nil {
		return fmt.Errorf("task postProcess failed, timerID: %d, runTimer: %d, err: %w\n", timerID, time.UnixMilli(unix), err)
	}
